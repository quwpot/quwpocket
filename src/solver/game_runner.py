...
def play_game(solver_depth, seed, max_turns=50):
    ...
    state = create_initial_state(p1_deck, p2_deck, seed=seed, energy_types=["Grass"], debug=True)
    ...
    while not state.game_over:
        ...
        print(f"len(hand) = {len(state.player1.hand)}")
        print(generate_actions(state))
